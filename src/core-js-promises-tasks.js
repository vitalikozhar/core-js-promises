...
function getFirstPromiseResult(...promises) {
  return Promise.race(promises[0]).then(
    (res) => res,
    (rej) => rej
  );
}
 ...
 */
async function queuePromises(prom) {
  const promises = prom;
  const arr = [];
  function func() {
    const result = promises[0].then((res) => {
      promises.shift();
      arr.push(res);
      if (promises.length !== 0) return func();
